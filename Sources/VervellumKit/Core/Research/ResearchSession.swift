...
/// One research thread's live run: the runner task, the queue of questions typed
/// while it was working, and the coalescer pacing its snapshots.
///
/// This is the unit `/new` and `/history` used to cancel. The engine and the panel
/// keep one session per open thread; leaving a thread detaches the session, which
/// keeps running — and keeps reporting — without touching the thread now on screen.
///
/// Nothing here marshals to a UI queue: `deliver` and `after` are injected so the
/// same type runs on `DispatchQueue.main` under AppKit and on the GLib main loop
/// under GTK — see the portability rule in AGENTS.md.
final class ResearchSession {
    ...
    /// The most questions that may wait at once. A queue with no visible end is a way
    /// to spend a provider's quota by accident.
    static let maxQueued = 3

    /// The thread this session is researching. A detached session keeps mutating it —
    /// that is the point — so a caller that wants the copy on screen must check it is
    /// looking at the session it thinks it is.
    ...
    /// transcript above conversation it had never seen.
    private(set) var queue: [QueuedQuestion] = []

    /// Set by `discard`. A discarded session takes no new questions and reports
    /// nothing further: the thread it was working on has been deleted, and persisting
    /// its last breaths would resurrect a thread the user just removed.
    ...
    /// The hop onto the caller's UI loop — `DispatchQueue.main.async` on macOS,
    /// `GTK.onMainLoop` on Linux. FIFO either way, so a run's snapshots land in the
    /// order they were produced.
    private let deliver: (@escaping () -> Void) -> Void
    private var task: Task<Void, Never>?
    private var runningTurnID: UUID?

    /// Rate-limits the streamed snapshots before they are published. See
    ...
    /// Called on the `deliver` queue whenever the visible state changes — a turn's
    /// contents or the queue. The front end repaints the thread if this session is
    /// the one on screen and persists `persistableThread` either way.
    var onChange: ((ResearchSession) -> Void)?

    /// Called when `isRunning` flips, in both directions. The engine uses it to keep
    /// the set of live sessions current; the panel uses it to flush the archive the
    /// moment a run settles.
    var onRunningChange: ((ResearchSession) -> Void)?

    /// Called with questions that were waiting and will not now be asked — after a
    /// Stop, or after a run that did not complete. The front end puts them back in
    /// the composer.
    ///
    /// Typed text is the one thing here the user cannot recover, so it is never simply
    /// dropped. Running the queue anyway would be worse: after a Stop it would ignore
    /// the Stop, and after a failure it would fire the rest of the queue at a provider
    /// that had just refused, spending a request per question to collect the same
    /// error again.
    ///
    /// The whole question goes back, attachments included, rather than its text: the
    /// bytes of a pasted screenshot are not on the user's disk anywhere and handing
    /// back the words alone would quietly lose them.
    var onQueueReturned: ((ResearchSession, [QueuedQuestion]) -> Void)?
    ...
    func ask(_ question: String, mode: Mode? = nil,
        ...
        guard !trimmed.isEmpty, !isDiscarded else { return .ignored }
        ...
        guard !isRunning else {
            ...
            // A queue change is a visible change: the chips under the thread just grew.
            onChange?(self)
            return .queued
        }
    ...
    func retry(_ id: UUID) {
        guard !isRunning, !isDiscarded,
              let turn = thread.turns.first(where: { $0.id == id }) else { return }
    ...
    private func start(_ trimmed: String, mode: Mode, attachments: [PendingAttachment] = [],
        ...
        let submitted = turn
        // Local turns — the notices `/help` and friends append — carry no question and
        // must not reach the model as conversation it supposedly had.
        ...
        task = Task { [weak self] in
            ...
            // snapshot is hopped onto the front end's queue in order. `async` is FIFO,
            // so the streamed chunks arrive in the order they were produced.
            let finished = await runner.run(submitted, mode: mode, history: history) { snapshot in
                deliver { self?.apply(snapshot) }
            }
            // The same queue as the snapshots, not a second hop. Mixing mechanisms means
            // the completion could be scheduled ahead of snapshots already queued — and
            // the final turn would then be overwritten by an earlier, partial one.
    ...
    ///
    /// Unlike `cancel` nothing is handed back and nothing is reported: the queue's
    /// questions belonged to a conversation that no longer exists, and a callback from
    /// here would only spend effort repainting a thread nobody can see. The task is
    /// still cancelled — deleting a thread should stop the work it paid for.
    func discard() {
        isDiscarded = true
        queue.removeAll()
        ...
        isRunning = false
        // Still reported: the engine keeps a set of running sessions, and a session
        // that went quiet here would stay in it forever.
        onRunningChange?(self)
    }
    ...
    /// A snapshot from the running turn. Runs on the `deliver` queue, which is the
    /// queue the coalescer is documented to live on.
    private func apply(_ snapshot: ResearchTurn) {
        guard !isDiscarded, snapshot.id == runningTurnID else { return }
    ...
    private func applyNow(_ snapshot: ResearchTurn) {
        guard !isDiscarded, snapshot.id == runningTurnID,
    ...
    private func finish(_ id: UUID, with turn: ResearchTurn) {
        // A cancelled or discarded run cannot overwrite Stop or a newer run's work.
        guard !isDiscarded, runningTurnID == id else { return }
    ...
    private func startNextQueued(after finished: ResearchTurn) {
        ...
        // `start` republishes, which covers the queue shrinking too.
        start(next.question, mode: next.mode, attachments: next.attachments)
    }

    // MARK: Local turns and persistence

    /// Appends a turn the pipeline never sees — `/help` output, an error shown in
    /// place. Local turns have no question and are filtered out of both history and
    /// `persistableThread`.
    ...
    /// The thread minus local turns. The archive writes this — a notice that exists
    /// only to answer a keystroke is not part of the conversation the user asked to
    /// keep, and a stored one would resurface as an empty bubble on reopen.
    var persistableThread: ResearchThread {
        var copy = thread
        copy.turns = copy.turns.filter { !$0.question.isEmpty }
        return copy
    }
